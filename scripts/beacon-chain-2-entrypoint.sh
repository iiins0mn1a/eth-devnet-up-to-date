...
# Beacon Chain 2 entrypoint script
...
if [ "$NS3_INTEGRATION" = "true" ]; then
    echo "[$(date)] NS-3 集成模式启动 beacon-chain-2"
...
if [ "$NS3_INTEGRATION" = "true" ]; then
    ...
    exec /beacon-chain \
      ...
      --p2p-host-ip=10.0.0.2 \
      --p2p-local-ip=10.0.0.2 \
      --p2p-tcp-port=13000 \
      --p2p-udp-port=12000 \
